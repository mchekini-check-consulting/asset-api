node("ci-node") {
    ...
    stage("Checkout") {
        checkout scmGit(branches: [[name: '*/develop']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/mchekini-check-consulting/asset-api.git']])
    }

    stage("Build") {
        sh "chmod 700 mvnw &&  ./mvnw package -DskipTests"
    }
